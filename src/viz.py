...
class Ax3DPose(object):
  def __init__(self, ax, lcolor="#3498db", rcolor="#e74c3c"):
    ...
    # I is the starting joint, J is the ending joint
  ...
  def update(self, channels, lcolor="#3498db", rcolor="#e74c3c"):
    ...
    for i in np.arange( len(self.I) ):
      """ By only visualizing the first seven joints, it removes all the other joints that 
      ...
      if (i<8): # 8 body parts
        ...
        # print(type(self.plots[i][0]))
      # self.plots[i][0].set_color(lcolor if (self.LR[i] and i<8)  else rcolor)
