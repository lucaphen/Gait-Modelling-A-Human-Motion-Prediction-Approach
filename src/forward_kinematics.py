...
def fkl( angles, parent, offset, rotInd, expmapInd ):
  ...
  ie. Converts kinematic tree to (x,y,z) coordinates of the system
...
def revert_coordinate_space(channels, R0, T0):
  ...
  Takes eucliden map input which is centered w.r.t. the starting point and convert it to 
  w.r.t. world co-ordinate space
  Adapted from
...
def main():
  ...
  ob = viz.Ax3DPose(ax) # Here it calls the vizualization code 
