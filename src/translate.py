...
These flags can be accessed through the cmd line. Would not need to change them manually.
...
# Architecture
tf.app.flags.DEFINE_string("model", "residual_unsup_SA", "Type of training model.")
...
newFile = "results_{0}_{1}.txt".format(FLAGS.model, FLAGS.action)
file = open(newFile, 'a')
    ...
    for _ in xrange( FLAGS.iterations ):
      ...
      if current_step % FLAGS.test_every == 0:
        ...
        if (current_step % 10000)==0:
          ...
          for ms in [80, 160, 320, 400, 560, 720, 880, 1000]:
            file.write(" {0:5d} |".format(ms))
        ...
        for action in actions:
          ...
          if (current_step % 10000)==0:
            ...
            for ms in [1,3,7,9,13,17,21,24]:
              if FLAGS.seq_length_out >= ms+1:
                file.write(" {0:.3f} |".format( mean_mean_errors[ms] ))
        ...
        print("============================\n"
              ...
              val_loss, srnn_loss))
        file.write("============================\nLearning rate:       {:.4f}\nTrain loss avg:      {:.4f}\nVal loss:            {:.4f}\nsrnn loss:           {:.4f}\n".format(model.learning_rate.eval(), loss, val_loss, srnn_loss))
        print()
