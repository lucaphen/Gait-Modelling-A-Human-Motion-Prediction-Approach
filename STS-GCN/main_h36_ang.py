...
file = open('results_{}.txt'.format(args.action), 'a')
print('total number of parameters of the network is: '+str(sum(p.numel() for p in model.parameters() if p.requires_grad)))
file.write("Total number of parameters of the network is: {} \n".format(sum(p.numel() for p in model.parameters() if p.requires_grad)))
...
def train():
  ...
  train_loss = []
  val_loss = []
  dataset = datasets.Datasets(args.data_dir,args.input_n,args.output_n,args.skip_rate, args.action, split=0 )
  print('>>> Training dataset length: {:d}'.format(dataset.__len__()))
  file.write("Training dataset length: {:d}\n".format(dataset.__len__()))
 
  data_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, num_workers=0, pin_memory=True)

  vald_dataset = datasets.Datasets(args.data_dir,args.input_n,args.output_n,args.skip_rate, args.action, split=1)
  print('>>> Validation dataset length: {:d}'.format(vald_dataset.__len__()))
  file.write("Validation dataset length: {:d}\n".format(vald_dataset.__len__()))
  ...
  for epoch in range(args.n_epochs):
      ...
      with torch.no_grad():
          ...
          for cnt,batch in enumerate(vald_loader):
              ...
              if cnt % 200 == 0:
                ...
                if (epoch+1)%50==0:
                  file.write('Training: [%d, %5d]  validation loss euler: %.3f validation loss mpjpe : %.3f\n' %(epoch + 1, cnt + 1, euler_loss.item(),mpjpe_loss))  
      ...
      # Used to show visual representation of training vs validation loss
        # plt.figure(1)
...
def test():
  ...
  for action in actions:
    ...
    n=0
    dataset_test = datasets.Datasets(args.data_dir,args.input_n,args.output_n,args.skip_rate, args.action, split=2)
    print('>>> test action for sequences: {:d}'.format(dataset_test.__len__()))
    file.write('Test dataset length: {:d}\n'.format(dataset_test.__len__()))
    ...
    n_batches+=n
  file.write("Testing: Overall average loss in euler angle is: {} overall average loss in mpjpe is: {}\n".format(accum_loss/n_batches, accum_loss_mpjpe/n_batches)) 
